...
class ConnectionStateMachine(state: State = AuthenticationRequired, val id: Int) extends StateMachine[Message, PgResponse, State] {
  ...
  transition {
    ...
    case (CommandComplete(DropTable | DropTrigger), SimpleQuery) => (None, EmitOnReadyForQuery(CommandCompleteResponse(1)))
  ...
  transition {
    ...
    case (CommandComplete(DropTable | DropTrigger), ExecutePreparedStatement) => (Some(CommandCompleteResponse(1)), Connected)
